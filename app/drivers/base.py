...
class BmcDriver:
    ...
    def console_session(self) -> tuple[str, str, str]:
        """Готовая сессия для встроенной консоли.

        Возвращает (имя cookie, значение, путь к странице консоли). Сервис
        ставит эту cookie на свой домен и отправляет туда браузер: cookie не
        различают порты, поэтому прокси консоли на соседнем порту получит её
        и отдаст BMC уже авторизованный запрос. Отдельный вход в BMC при этом
        исчезает.
        """
        raise NotImplementedError
